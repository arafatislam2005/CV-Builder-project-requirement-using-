...
export default function ReferencesSection({ data, onChange }) {
  ...
  const addItem = () => onChange([...data, { ...emptyRef }]);
  const removeItem = (index) => onChange(data.filter((_, i) => i !== index));

  return (
    <div className="bg-white border border-gray-200 rounded-lg p-6 shadow-sm">
      <h3 className="text-lg font-semibold mb-4 pb-3 border-b border-gray-200">References</h3>
          ...
          <div key={index} className="p-4 border border-gray-200 rounded-lg bg-gray-50 relative">
            ...
            <div className="grid grid-cols-2 gap-4 max-sm:grid-cols-1">
              <div className="mb-3">
                ...
                <input className={inputCls} value={item.name} onChange={(e) => handleItemChange(index, 'name', e.target.value)} placeholder="Reference full name" />
              </div>
              <div className="mb-3">
                <label className="block text-sm font-semibold text-gray-700 mb-1.5">Position</label>
                <input className={inputCls} value={item.position} onChange={(e) => handleItemChange(index, 'position', e.target.value)} placeholder="e.g. Senior Manager" />
              </div>
            </div>
            <div className="grid grid-cols-2 gap-4 max-sm:grid-cols-1">
              <div className="mb-3">
                <label className="block text-sm font-semibold text-gray-700 mb-1.5">Company</label>
                <input className={inputCls} value={item.company} onChange={(e) => handleItemChange(index, 'company', e.target.value)} placeholder="Company name" />
              </div>
              <div className="mb-3">
                <label className="block text-sm font-semibold text-gray-700 mb-1.5">Email</label>
                <input type="email" className={inputCls} value={item.email} onChange={(e) => handleItemChange(index, 'email', e.target.value)} placeholder="reference@email.com" />
              </div>
            </div>
            <div>
              <label className="block text-sm font-semibold text-gray-700 mb-1.5">Phone</label>
              <input className={inputCls} value={item.phone} onChange={(e) => handleItemChange(index, 'phone', e.target.value)} placeholder="+971 XX XXX XXXX" />
            </div>
          </div>
        ))}
      </div>
